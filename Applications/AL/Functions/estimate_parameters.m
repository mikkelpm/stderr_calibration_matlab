function estim = estimate_parameters(mu_hat, V_hat)
    ...
    estim.justid.indep_se = sqrt(diag(jacob_justid*diag(diag(V_hat(1:3,1:3)))*jacob_justid')); % SE if independent
    ...
    h_theta_hat_justid = h(estim.justid.theta_hat);
    estim.overid_test.moment_errors = mu_hat'-h_theta_hat_justid;
    G_hat_justid = G_fct(estim.justid.theta_hat); % hat{G}
    M_hat_justid = eye(5) - G_hat_justid*[jacob_justid, zeros(3,2)]; % Jacobian for transforming sample moments into moment errors
    estim.overid_test.se = sqrt(diag(M_hat_justid*V_hat*M_hat_justid'));
    estim.overid_test.wcse = abs(M_hat_justid)*sigma_hat;
    
    % Can't test validity of first three moments
    ...
    for i=1:k % Loop over parameters
        [~, res_onestep] ...
            = HTest_Single_WorstCaseOptW_Step2(V_hat, ...
                ...
                false, h, [], ...
                true, mu_hat', estim.justid.theta_hat, h_theta_hat_justid);
        estim.wcopt.theta_hat(i) = res_onestep.lcomb;
        estim.wcopt.wcse(i) = res_onestep.stderr_check;
        estim.wcopt.x_hat(i,:) = res_onestep.x_check;
    end


    %% Full-information efficient estimation

    estim.fullinfo.theta_hat = getOnestep(h_theta_hat_justid, inv(V_hat), G_hat_justid, estim.justid.theta_hat, mu_hat');
    estim.fullinfo.se = sqrt(diag(inv(G_hat_justid'*(V_hat\G_hat_justid))));
