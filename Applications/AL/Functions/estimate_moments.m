function [sample_moments, sample_moments_var] = estimate_moments(change, price)
    ...
    X = change.*[ones(n,1) price.^[2 4] abs(price).^[1 2]];
    ...
    sample_moments(2:5) = sample_moments(2:5)/sample_moments_uncond(1);
    the_eye = eye(5);
    jacob = [the_eye(1,:); [-sample_moments_uncond(2:5)'/sample_moments_uncond(1)^2, the_eye(2:end,2:end)/sample_moments_uncond(1)]];
